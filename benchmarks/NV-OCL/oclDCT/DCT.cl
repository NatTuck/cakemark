...
// Note that the workgroup size could be set by the host program rather than
// as an attribute, so this does not invalidate the specialization test.
#define CBS 8
...
__kernel __attribute__((reqd_work_group_size(BLOCK_X, BLOCK_Y / CBS, 1)))
void DCT8x8(
...
/* @spec DCT8x8(stride, imageH, imageW, BLOCK_SIZE) */
{
...
/* @spec(stride, imageH, imageW, BLOCK_SIZE) */
{
    __local float l_Transpose[BLOCK_Y][BLOCK_X + 1];
    const uint    localX = get_local_id(0);
    const uint    localY = BLOCK_SIZE * get_local_id(1);
