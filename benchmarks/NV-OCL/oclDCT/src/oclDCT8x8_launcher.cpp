...
extern "C" void initDCT8x8(cl_context cxGPUContext, cl_command_queue cqParamCommandQue, const char **argv){
    ...
    if (ciErrNum != CL_SUCCESS) {
        ...
        cl_device_id dev = oclGetFirstDev(cxGPUContext);

        int ee;
        ee = clGetProgramBuildInfo(cpDCT8x8, dev, CL_PROGRAM_BUILD_LOG, 0, 0, &log_size);
        printf("Error: %d\n", ee);
        assert(ee == CL_SUCCESS);

        log_text = (char*) alloca(log_size);

        ee = clGetProgramBuildInfo(cpDCT8x8, dev, CL_PROGRAM_BUILD_LOG, log_size, log_text, 0);
        printf("Error: %d\n", ee);
        assert(ee == CL_SUCCESS);
...
extern "C" void DCT8x8(
    ...
    ciErrNum |= clSetKernelArg(ckDCT, 5, sizeof(cl_uint), (void*)&bs);
    shrCheckError(ciErrNum, CL_SUCCESS);
