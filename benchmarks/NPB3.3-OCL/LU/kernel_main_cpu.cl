...
/* @spec rhsz(nx, ny, nz) */
{
  ...
  int my_id = (j-jst)*ISIZ1 + (i-ist);
  int my_offset = my_id * ISIZ1*5;
  flux = (__global double (*)[5])&g_flux[my_offset];

  my_offset = my_id * ISIZ3*6;
  utmp = (__global double (*)[6])&g_utmp[my_offset];
  my_offset = my_id * ISIZ3*5;
  rtmp = (__global double (*)[5])&g_rtmp[my_offset];
