...
/* @spec checksum(d1, d2) global_size(0) */
{
  ...
  if (j <= 1024) {
    ...
    r = 3*j % NY;
    s = 5*j % NZ;
    l_chk[lid] = u1[s*d2*(d1+1) + r*(d1+1) + q];
  } else {
...
/* @spec cffts2(d1, d2, d3, logd2) global_size(0, 1) */
{
  ...
  for (j = 0; j < d2; j++) {
    ty1[j] = x[ki_idx + j*(d1+1)];
  }

  cfftz(is, logd2, d2, u, ty1, ty2);

  for (j = 0; j < d2; j++) {
    xout[ki_idx + j*(d1+1)] = ty1[j];
  }
}
...
/* @spec cffts3(d1, d2, d3, logd3) global_size(0, 1) */
{
  ...
  int ji_idx = j*(d1+1) + i;
  for (k = 0; k < d3; k++) {
    ty1[k] = x[k*d2*(d1+1) + ji_idx];
  }

  cfftz(is, logd3, d3, u, ty1, ty2);

  for (k = 0; k < d3; k++) {
    xout[k*d2*(d1+1) + ji_idx] = ty1[k];
  }
}
