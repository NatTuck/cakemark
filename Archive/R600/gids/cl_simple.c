...
unsigned clSimpleInitGpuDevice(cl_device_id * device_id)
{
   ...
   char* plat_name = alloca(96);
   error = clGetPlatformInfo(platform_id, CL_PLATFORM_NAME, 96, (void*)plat_name, 0);
   if (error != CL_SUCCESS) {
      fprintf(stderr, "clGetPlatformInfo(CL_PLATFORM_NAME) failed: %s\n", 
              clUtilErrorString(error));
      return 0;
   }
   ...
   char* dev_name = alloca(96);
   error = clGetDeviceInfo(*device_id, CL_DEVICE_NAME, 96, dev_name, 0);
   if (error != CL_SUCCESS) {
      fprintf(stderr, "clGetDeviceInfo(CL_DEVICE_NAME) failed: %s\n", 
              clUtilErrorString(error));
      return 0;
   }
